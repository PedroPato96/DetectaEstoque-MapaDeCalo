...
LIMITE_VAGA_OCUPADA = 3000
NUM_VAGAS = len(VAGAS)
DELAY = 50

# Matriz para armazenar contagens de ocupação
heatmap_data = np.zeros((NUM_VAGAS,), dtype=np.int32)
...
def verifica_vagas(img_dil, vagas, heatmap_data):
    """
    Verifica o status e atualiza o mapa de calor.
    """
...
def gerar_mapa_de_calor(heatmap_data, vagas):
    """
    Gera e exibe um mapa de calor com base nos dados de ocupação.
    """
    altura = max(y + h for x, y, w, h in vagas)
    largura = max(x + w for x, y, w, h in vagas)
    ...
    plt.title("Mapa de Calor do displays de produtos")
    plt.show()
...
def main():
    ...
    while True:
        ...
        verifica_vagas(img_dil, VAGAS, heatmap_data)
    ...
    gerar_mapa_de_calor(heatmap_data, VAGAS)
